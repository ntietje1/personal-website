    ...
    <div className={`absolute inset-0 pointer-events-none ${className}`}>
      {stars.map((star) => (
        <div
          key={star.id}
          className={`absolute ${
            star.type === 'star1' ? 'star1-sprite' : 
            star.type === 'star2' ? 'star2-sprite' : 'comet-sprite'
          }`}
          style={{
            left: `${star.x}%`,
            top: `${star.y}%`, // Use percentage positioning relative to the layer
            animation: `${
              star.type === 'star1' ? 'star1-anim' : 
              star.type === 'star2' ? 'star2-anim' : 'comet-anim'
            } ${star.animationDuration}s steps(${
              star.type === 'star1' ? 8 : 
              star.type === 'star2' ? 7 : 8
            }, end) infinite`,
            animationDelay: `${star.animationDelay}s`,
            transform: star.rotation ? `scale(2.5) rotate(${star.rotation}deg)` : undefined,
          }}
        />
      ))}
    </div>
